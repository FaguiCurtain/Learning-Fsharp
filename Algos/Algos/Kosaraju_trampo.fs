...
// remplacer les ('r) et (unit -> 'r) par 'r response
//                    ou ('a   -> 'r)
// remplacer les responses explicites par Continue (fun (...))
...
let DFSLoop1 (G:DFSgraph1)  = 
     ...
     let rec iter_t (n:int) (f_t:'a->('r response)->'r response) (list:'a list) (cont: 'r response) : 'r response = 
         match list with 
            | [] -> (t <- t+1) ; (G.[n].finishingtime <- t) ; cont
            | x::xs -> f_t x (Continue (fun ()-> iter_t n f_t xs cont)) // attention à la syntaxe
     let rec DFSsub_t (G:DFSgraph1) (n:int) (cont: 'r response) : 'r response=  
          let my_f_t (j:int)(cont:'r response):'r response =
     ...
     for i in maxnum_nodes .. -1 .. 1 do
       // printfn "%d" i
        if not(G.[i].explored1) then do 
                                    s <- i
                                    trampoline ( Continue(fun()-> DFSsub_t G i (Done()) ) )
...
let DFSLoop2 (G:DFSgraph2)  = 
     ...
     let rec DFSsub_t (G:DFSgraph2) (n:int) (cont:'r response) : 'r response=  
          let my_f_t (j:int)(cont:'r response):'r response = if not(G.[j].explored2) then Continue(fun ()->(DFSsub_t G j cont)) else cont
          G.[n].explored2 <- true
          G.[n].leader <- s         
          iter_t n my_f_t G.[n].children cont
