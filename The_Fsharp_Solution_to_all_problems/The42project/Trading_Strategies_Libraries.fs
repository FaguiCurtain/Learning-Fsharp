...
let trade_list = new Dictionary<string,(DateTime*EntryLvl*ExitLvl) []>()

for pair in Currency_list do
    let (_5mnchart,hourlychart,HourlyRSI,S,res1A) = strategy_analysis pair 300 0.95

    let ans = res1A |> fun (a,b,c) ->c |> List.map (fun (a,b,c,d)->(b,c,d)) |> List.toArray

    trade_list.[pair] <- ans

// type trade_record = {Pair:string;TradeDate:DateTime;EntryLvl:EntryLvl;ExitLvl:ExitLvl}
    ...
    printfn "pair = %A" pair
    printfn "number of trades=%A" (Array.length trade_dates_bull1)
...
let all_trades = [ for pair in Currency_list do 
                                              // yield trade_list.[pair] |> Array.map (fun (a,b,c)-> {Pair = pair;TradeDate=a;EntryLvl=b;ExitLvl=c})
                                              yield trade_list.[pair] |> Array.map (fun (a,b,c)-> (pair,a,b,c))
                  ]
                 |> Array.concat
                 |> Array.sortBy (fun (pair,date,_,_)->(date,pair))
                 |> Array.map (fun (a,b,c,d)->(b,a,c,d))

let perf : float[]= all_trades |> Array.map (fun (_,_,c,d)->(d-c)/c)
let avg_return = (Array.sum perf)/float (Array.length perf)
